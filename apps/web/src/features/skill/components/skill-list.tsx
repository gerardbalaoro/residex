...
import { BadgeCheckIcon, LoaderCircleIcon, PlusIcon, Trash2Icon, XIcon } from "lucide-react";
...
export function SkillList({ rows, search, isLoading = false, onCreate, onEdit }: Props) {
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      await deleteSkill({ id: [...selectedIds] });
      setSelectedIds(new Set());
    } finally {
      setIsBulkDeleting(false);
      setConfirmBulkDelete(false);
    }
  }, [deleteSkill, selectedIds]);

  if (isLoading) {
    return (
      <ItemGroup>
        {Array.from({ length: SKELETON_ROW_COUNT }).map((_, index) => (
          <SkillListItem.Skeleton key={index} />
        ))}
      </ItemGroup>
    );
  }
    ...
    return (
      <Empty className="border">
        <EmptyHeader>
          ...
          <EmptyTitle>
            {search.trim() ? "No skills found" : "You haven't added any skills"}
          </EmptyTitle>
          <EmptyDescription>
            {search.trim()
              ? "Try a different search term, or add a new skill."
              : "Track surgical and communication skills you want to develop across encounters."}
          </EmptyDescription>
        </EmptyHeader>
        <EmptyContent>
      ...
      >
        <AlertDialogContent size="sm">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes the selected skills from your active list.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
